...
class UserAccountService {
  ...
  Future<Map<String, dynamic>> getUserAccount(BuildContext context) async {
    ...
    try {
      ...
      final rekeningDetail = await transactionService.getRekeningPengguna(userSlug, noRekPengguna);
      final riwayatTransaksiData = await transactionService.getTransactionHistory(userSlug, noRekPengguna);
      ...
      if (rekeningDetail != null && rekeningDetail.isNotEmpty) {
        ...
        if (riwayatTransaksiData != null && riwayatTransaksiData.isNotEmpty) {
          final pengguna = await transactionService.getNamaPenggunaByIdRekening(rekening['id']);
          result['namaPengguna'] = pengguna?['pengguna'];
          
          result['riwayatTransaksi'] = riwayatTransaksiData
              .where((transaction) =>
                  transaction['status_transaksi'].toString().toLowerCase() != 'pending')
              .take(5)
              .toList();
        }
      }
